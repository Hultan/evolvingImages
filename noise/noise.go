...
// TypeOfNoise indicates which noise MakeNoise will generate
type TypeOfNoise int

const (
	FBM TypeOfNoise = iota
	TURBULENCE
)

// Turbulence generates turbulent fractal noise
func Turbulence(x, y, frequency, lacunarity, gain float64, octaves int) float64 {
	var sum float64
	amplitude := float64(1)
...
func MakeNoise(noiseType TypeOfNoise, frequency, lacunarity, gain float64, octaves, w, h int) (noise []float64, min, max float64) {
	...
	minMaxChan := make(chan float64, numRoutines*2)
	...
	for i := 0; i < numRoutines; i++ {
		go func(i int) {
			defer wg.Done()
			innerMin := math.MaxFloat64
			innerMax := -math.MaxFloat64
			start := i * batchSize
			...
			for j := start; j < end; j++ {
				...
				if noiseType == TURBULENCE {
					noise[j] = Turbulence(float64(x), float64(y), frequency, lacunarity, gain, octaves)
				} else if noiseType == FBM {
					noise[j] = Fbm2(float64(x), float64(y), frequency, lacunarity, gain, octaves)
				}
...
// Snoise2 generates 2D simplex noise for the coordinates provided
func Snoise2(x, y float64) float64 {

	const F2 float64 = 0.366025403 // F2 = 0.5*(sqrt(3.0)-1.0)
	const G2 float64 = 0.211324865 // G2 = (3.0-Math.sqrt(3.0))/6.0
